...
public:
	...
	Mat operator+(const double &);	//矩阵加一个常数

	Mat operator-(Mat &);	//两矩阵相减
	Mat operator-(const double &);	//矩阵减一个常数

	Mat operator*(Mat &);	//矩阵乘一般乘法
	Mat operator*(const double &);	//矩阵乘以一个常数

	Mat operator/(Mat &);	//矩阵叉除，矩阵乘另一个矩阵的逆
	Mat operator/(const double &);	//矩阵除以一个常数

	void eye(const long &);		//产生n*n的单位矩阵
	Mat dotMultiplication(const Mat &);	//点积，矩阵元素分别对应乘
	...
	Mat mean(const int &);	//n=1或2，为1时求每一列的平均值，返回为一行原列的矩阵；为2时求每一行的平均值，返回为一列原行的矩阵
	Mat reshape(const long &,const long &);		//改变矩阵的行和列，必须满足“原行*原列=新行*新列”
	Tensor *reshape(Mat);		//将矩阵改变为以Mat为索引的张量，这里Mat为一行n列，张量为n维，每一维的数据的个数为Mat对应的数值
	long prod();		//返回矩阵中所有元素的乘积
	long prod(const long &,const long &);	//返回除了（long,long）这个位置之外的矩阵所有元素的乘积
	...
	void MatCopy(const double*,const long &,const long &);	//从一个数组中拷贝数据到Mat中
	void ReadFileData(const string &);			//从文件中读取数据到Mat中

	void print();				//输出矩阵的元素
	Mat repmat(const long &,const long &);		//把矩阵元素周期扩展
	Tensor *tensorize(const long &,Mat);

	Mat wshift(const long &);		//把Mat循环左移n位，暂时只支持一行的矩阵


private:
	void create(const long &,const long &);		//真正的初始化
